...
const getFriendRequest = async (req, res) => {
    try {
        ...
        let existRequest = await FriendRequest.aggregate([
            {
                $match: { status: 'pending', req_receiver_id: new mongoose.Types.ObjectId(userId) }
            },
            {
                $lookup: {
                    from: 'users',
...
const updateRequest = async (req, res) => {
    try {
        ...
        let senderRequest = {};
        if (req_sender_id) senderRequest.friends.push(sender._id)
        ...
        let receiverRequest = {};
        if (req_sender_id) receiverRequest.friends.push(receiver._id)
        await upsert(userModel, receiver._id, senderRequest);
